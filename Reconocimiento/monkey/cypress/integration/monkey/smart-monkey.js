...
function loginIfNeeded() {
  cy.url().then((currentUrl) => {
    ...
    if (currentUrl.includes("/signin")) {
      // Verifica si está en la página de inicio de sesión
      cy.get('input[name="identification"]').type(
        "af.velezv1@uniandes.edu.co",
        {
          log: false,
        }
      );
      cy.get('input[name="password"]').type("ABC1234abc", { log: false });
...
Cypress.on("fail", (error, runnable) => {
  if (error.message.includes("cy.scrollTo()")) {
    Cypress.log({ name: "Scroll Error", message: error.message });
